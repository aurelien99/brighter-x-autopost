...
Architecture :
  ...
  3. Generation du thread via IA (Claude)
...
# Local
from config import (
    RSS_FEED_URL, CHANNEL_NAME, CHANNEL_HANDLE,
    X_USERNAME, X_USER_DISPLAY,
    ...
    ARTIFACT_APPROVED_TOKEN, ARTIFACT_REJECTED_TOKEN,
    ARTIFACT_EDIT_REQUESTED_TOKEN,
...
AIBot = anthropic.Anthropic(api_key=AI_KEY)
...
def load_json(path: str, default: Any) -> Any:
    """
    Charge un fichier JSON. Si le fichier n'existe pas,
    retourne la valeur par defaut et l'ecrit sur disque.
    """
...
def fetch_latest_video() -> Optional[Dict[str, str]]:
    """
    Interroge le flux RSS de la chaine et retourne
    les metadonnees de la dernière video.
    """
...
def extract_transcript_with_fallback(video_id: str) -> str:
    """
    Tente l'extraction du transcript (auto ou manuel) avec un
    fallback sur un transcript synthetique si l'API echoue.
    """
...
def generate_thread_with_ai(
    title: str, transcript: str, link: str
) -> List[str]:
    """
    Appelle Claude avec le Prompt Master et retourne
    la liste des tweets (3 a 8).
    """
    logger.info("Generation du thread via IA...")
    ...
    response = AIBot.messages.create(
        model=AI_MODEL,
    ...
    logger.info("Response brute IA recue.")
    ...
    for line in lines:
        if len(line) <= 260 and len(line) >= 20:
            tweets.append(line)
    if not tweets:
        logger.error("Aucun tweet genere par l'IA.")
...
def save_artifact(video_id: str, tweets: List[str]) -> None:
    ...
    Utilise un token en premiere ligne pour la validation.
    Approve si le token est APPROVED/EDIT_REQUESTED.
    """
...
def load_artifact(video_id: str) -> tuple[bool, List[str]]:
    ...
    Si le token n'est pas APPROVED ou EDIT_REQUESTED, le fichier est supprime.
    """
    path = compute_artifact_path(video_id)
    if not path.exists():
        return False, []
...
def review_thread(tweets: List[str], video_id: str) -> List[str]:
    """
    Affiche chaque tweet avec son nombre de caracteres.
    Demande a l'utilisateur:
      y -> publier
      n -> skip
      edit -> teleporter dans le fichier artifact
    """
    print("\n" + "="*60)
...
def print_env_diagnosis():
    ...
    required = ['X_API_KEY', 'X_API_SECRET', 'X_ACCESS_TOKEN', 'X_ACCESS_SECRET', 'ANTHROPIC_API_KEY']
    print("\nENV DIAGNOSIS:")
    for k in required:
        if os.environ.get(k):
            print(f"  {k}: OK")
...
def check_for_new_video() -> Optional[Dict[str, str]]:
    """
    Interroge le RSS et ne retourne la video que si elle
    est nouvelle (non dans POSTED_DB ni dans PLANNING_DB).
    """
